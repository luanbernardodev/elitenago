...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#EEDC9A', font = '30px sans-serif' }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    const isMobile = this.screen.width < 768;
    const heightRatio = isMobile ? 0.54 : 0.62;
    this.plane.scale.y = this.viewport.height * heightRatio;
    const cardAspect = isMobile ? 0.72 : 0.76;
...
class App {
  ...
  onTouchUp(_e: MouseEvent | TouchEvent) {
    ...
    if (!this.hasMoved && this.onItemClick) {
      if (this.medias && this.medias[0]) {
        ...
        const centerIndex = Math.round(Math.abs(this.scroll.current) / width) % (this.mediasImages.length / 2);
        const item = this.mediasImages[centerIndex] || this.mediasImages[0];
        this.onItemClick(item, centerIndex);
      } else {
        this.onItemClick({ image: '', text: '' }, 0);
      }
    }
  }
  ...
  addEventListeners() {
    ...
    // Scoped specifically to container to prevent capturing global page clicks
    if (this.container) {
    ...
    // Window listeners ONLY for dragging continuation when isDown is active
...
export function CircularGallery({
  ...
  return (
    <div
      ...
      className={`w-full h-full overflow-hidden cursor-grab active:cursor-grabbing select-none outline-none focus:outline-none focus:ring-0 focus-visible:outline-none border-none ${className}`}
      style={{ WebkitTapHighlightColor: 'transparent', outline: 'none' }}
      ref={containerRef}
      tabIndex={-1}
      role="region"
      aria-label="Circular image gallery. Drag to scroll, click to open full gallery."
    />
  );
}
