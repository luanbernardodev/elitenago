...
const DEFAULT_IMAGES: ImageItem[] = [
  {
    ...
    alt: 'Capoeira Movimento e Roda'
  },
  {
    src: 'https://images.unsplash.com/photo-1518611012118-696072aa579a?q=80&w=900&auto=format&fit=crop',
    alt: 'Treino e Arte Marcial'
  },
  {
    src: 'https://images.unsplash.com/photo-1574680096145-d05b474e2155?q=80&w=900&auto=format&fit=crop',
    alt: 'Mestres e Tradição'
  },
  {
    src: 'https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad?q=80&w=900&auto=format&fit=crop',
    alt: 'Música e Berimbau'
  },
  {
  ...
  },
  {
    ...
    alt: 'Capoeira Expressão Cultural'
  },
  {
...
export function DomeGallery({
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    const maxTargetH = Math.min(window.innerHeight * (isMobile ? 0.68 : 0.75), 640);
    const targetSize = Math.min(maxTargetW, maxTargetH);

    const mainWidth = mainR.width;
    const mainHeight = mainR.height;
    const centeredLeft = Math.max(0, (mainWidth - targetSize) / 2);
    const centeredTop = Math.max(0, (mainHeight - targetSize) / 2);
    ...
    const onFirstEnd = (ev: TransitionEvent) => {
      ...
      const tempWidth = targetSize;
      const tempHeight = targetSize;

      overlay.style.transition = `left ${enlargeTransitionMs}ms ease, top ${enlargeTransitionMs}ms ease, width ${enlargeTransitionMs}ms ease, height ${enlargeTransitionMs}ms ease`;
      requestAnimationFrame(() => {
        overlay.style.left = `${centeredLeft}px`;
        overlay.style.top = `${centeredTop}px`;
        overlay.style.width = `${tempWidth}px`;
        overlay.style.height = `${tempHeight}px`;
      });
      const cleanupSecond = () => {
      ...
      >
        <main
          ...
          style={{
            ...
            backgroundColor: overlayBlurColor === '#050505' ? 'transparent' : overlayBlurColor
          }}
        >
          <div className="stage">
          ...
          <div
            ...
            style={{
              backgroundImage: `radial-gradient(rgba(0, 0, 0, 0) 60%, rgba(5, 5, 5, 0.4) 100%)`
            }}
          />
