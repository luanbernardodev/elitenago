                ...
                >
                  ...
                  {desktopViewMode === 'list' ? (
                    <BorderGlow
                      ...
                      className="p-5 sm:p-7 group hover:shadow-2xl transition-all duration-300"
                    >
                    ...
                    <BorderGlow
                      ...
                      backgroundColor="#08080a"
                      colors={['#F6E7B8', '#EED89F', '#E3C887']}
                      className="p-5 sm:p-7 h-full flex flex-col justify-between group hover:shadow-2xl transition-all duration-300"
                    >
                      <div>
                        {/* Badge & Date */}
                      ...
                      <div className="pt-4 border-t border-white/10 flex items-center justify-between text-xs font-bold uppercase tracking-wider text-neutral-200">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="w-7 h-7 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-all shadow-sm">
                            <ArrowRight className="w-3.5 h-3.5" />
                          </div>
                        </div>
                      </div>
      ...
      {selectedNews &&
        createPortal(
          ...
          >
            <div
              data-lenis-prevent="true"
              className="max-w-3xl w-full max-h-[88vh] my-auto overflow-y-auto modal-scrollbar overscroll-contain rounded-2xl sm:rounded-3xl p-6 sm:p-10 relative border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.85)] space-y-6 bg-[#0c0c10] text-white select-text"
            >
              {/* Close Button */}
              <button
              ...
              <div className="space-y-3 pr-10">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-neutral-200 text-xs border border-white/15">
                    <ShieldCheck className="w-3.5 h-3.5 text-[#EEDC9A]" />
                    {selectedNews.category}
                  </span>

                  <div className="flex items-center gap-1.5 text-xs text-neutral-400 font-light ml-auto">
                    <Calendar className="w-3.5 h-3.5 text-neutral-500" />
                    <span>{selectedNews.date}</span>
                  </div>
                </div>

                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black font-syne text-white leading-tight">
                  {selectedNews.title}
                </h2>
              </div>

              {/* Full Description & Paragraphs */}
              <div className="prose prose-invert prose-amber max-w-none text-sm sm:text-base text-neutral-300 font-light leading-relaxed space-y-4 pt-6 border-t border-white/10 whitespace-pre-line">
                <p>{selectedNews.content}</p>

                <div className="p-4 rounded-xl bg-white/[0.03] border border-white/10 text-xs sm:text-sm text-neutral-300 font-medium">
                  💡 Para maiores informações, inscrições em oficinas ou credenciamento de caravanas, fale diretamente com a coordenação do Grupo Elite Nagô pelo WhatsApp oficial.
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-white/10 flex flex-wrap items-center justify-between gap-4">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 text-neutral-300 text-xs font-semibold border border-white/10 transition-colors cursor-pointer"
                >
                  <Share2 className="w-4 h-4 text-[#EEDC9A]" />
                  <span>{copiedId === selectedNews.id ? 'Link Copiado!' : 'Compartilhar Notícia'}</span>
                </button>

                <button
                  type="button"
                  onClick={() => setSelectedNews(null)}
                  className="px-6 py-2.5 rounded-full bg-white hover:bg-[#EEDC9A] text-neutral-950 hover:text-black text-xs font-black font-syne uppercase tracking-wider shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all cursor-pointer"
                >
                  Fechar Leitura
                </button>
              </div>
